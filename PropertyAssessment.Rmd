...
null_counts <- colSums(is.na(history))
print(null_counts)
#now replacing numerical null values with median and categorical null values with mode
...
replace_nulls_with_unknown <- function(data) {
  data %>%
    mutate(across(where(is.factor), ~ifelse(is.na(.), "Unknown", .)))
}
history <- replace_nulls_with_unknown(history)
